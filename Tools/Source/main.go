...
func getAsReader(str string) string {
	...
	case "bool":
		return "bytes.readUnsignedByte() == 1"
	case "short":
...
func getCSType(str string) string {
	if str == "string" {
		return "string"
	}
	return str
}
func getCSReader(str string) string {
	switch str {
	case "byte":
	...
	case "string":
		return "bytes.ReadString(bytes.ReadInt16())"
	}
	return ""
}
func checkInputType(str string) bool {
	switch str {
	case "byte", "bool", "short", "int", "string":
		return true
	default:
		return false
	}
...
func createCS(tbl *TblNode) {
	content, err := getContent("template.cs")
	if err != nil {
		fmt.Println("template.cs is not exist!")
	}
	contents := strings.Split(content, "||")
	fileTpl := contents[0]
	varTpl := contents[1]
	...
	//fileTpl = strings.Replace(fileTpl, "$ItemNum", strconv.Itoa(tbl.ItemNum), -1)
	if tbl.Key != "" {
		fileTpl = strings.Replace(fileTpl, "$DicType", "Dictionary<"+tbl.KeyType+", "+clsName+">", -1)
		fileTpl = strings.Replace(fileTpl, "$DicAdd", contents[3], -1)
		fileTpl = strings.Replace(fileTpl, "$DicKey", tbl.Key, -1)
		fileTpl = strings.Replace(fileTpl, "$KeyType", tbl.KeyType, -1)
		fileTpl = strings.Replace(fileTpl, "$ifContain", contents[5], -1)
	} else {
		fileTpl = strings.Replace(fileTpl, "$DicType", "List<"+clsName+">", -1)
		fileTpl = strings.Replace(fileTpl, "$DicAdd", contents[4], -1)
		fileTpl = strings.Replace(fileTpl, "$KeyType", "int", -1)
		fileTpl = strings.Replace(fileTpl, "$ifContain", contents[6], -1)
	}
	...
	for _, val := range tbl.Items {
		newVar := strings.Replace(varTpl, "$Info", val.Name, -1)
		newVar = strings.Replace(newVar, "$Prop", val.Code, -1)
		newVar = strings.Replace(newVar, "$Type", getCSType(val.Type), -1)
		vars = append(vars, newVar)

		newRead := strings.Replace(readTpl, "$Prop", val.Code, -1)
		newRead = strings.Replace(newRead, "$Reader", getCSReader(val.Type), -1)
		readers = append(readers, newRead)
	}
	fileTpl = strings.Replace(fileTpl, "$Prop", strings.Join(vars, ""), -1)
	fileTpl = strings.Replace(fileTpl, "$Reader", strings.Join(readers, ""), -1)
...
func main() {
	...
	for _, tbl := range v.Data {
		...
		for i := 0; i < count; i++ {
			...
			if tbl.Key != "" && tbl.Items[i].Code == tbl.Key {
				tbl.KeyType = tbl.Items[i].Type
			}
		}
		for idx, headVal := range head.Cells {
			for i := 0; i < count; i++ {
		...
		binary.Write(dataBuf, binary.BigEndian, int16(len(clsName)))
		dataBuf.WriteString(clsName)
		bytesBuf := buf.Bytes()
		binary.Write(dataBuf, binary.BigEndian, int16(len(bytesBuf)))
		dataBuf.Write(bytesBuf)
		//datas = append(datas, ConfigBytes{Key: filePath, Data: buf.Bytes()})

		fmt.Printf("bytes len:%d\n", len(buf.Bytes()))
	}

	startIdx := strings.LastIndex(v.Out, "/") + 1
	filePath := v.Out[0:startIdx]
	fmt.Printf("save %s at %s\n", v.Out, filePath)
	os.MkdirAll(filePath, os.ModeDir)
	errFile := ioutil.WriteFile(v.Out, dataBuf.Bytes(), 0644)
	if errFile != nil {
		fmt.Println("error:", errFile)
	}
}
